		...
		var init = function() {
			...
			for (i=0; i<$('.animate-in').length; i++) {
				...
				switch (anim.attr('data-animation')) {
					...
					case 'fly-in-bottom':
						anim
							.parent().css('overflow','hidden');
						scrolldeck.controller.animate(anim, { delay: windowHeight/2, duration: windowHeight/2, property:'bottom', start:-1200 });
						break;
					case 'space-in':
			...
			for (i=0; i<$('.animate-build').length; i++) {
				...
				switch (anim.attr('data-animation')) {
					...
					case 'fly-in-bottom':
						anim.parent().css('overflow','hidden');
						scrolldeck.controller.animate(anim, { delay: (anim.attr('data-build')-1)*400, duration: 400, property:'bottom', start:-1200, pin:true });
						break;
					case 'space-in':
